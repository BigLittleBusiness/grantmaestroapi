# GrantMaestro Architecture Overview

This document explains what has been built for GrantMaestro, how the parts connect, and why the main design decisions were made. It is written for readers who do not need to know AWS terminology.

## Executive Summary

GrantMaestro now has two working environments:
...
The main marketing domain `grantmaestro.com` remains separate and is not managed by the new app frontend stack.

## Environment Overview

| Purpose | UAT | Production |
|---------|-----|------------|
| Frontend app URL | `https://app.uat.grantmaestro.com` | `https://app.grantmaestro.com` |
| Backend API URL | `https://api.uat.grantmaestro.com` | `https://api.grantmaestro.com` |
| Frontend storage bucket | `uat.grantmaestro.com-frontend` | `prod.grantmaestro.com-frontend` |
| Backend service | `grantmaestro-uat-uat` | `grantmaestro-prod-prod` |
| Database | `grantmaestro_uat` | `grantmaestro` |
| Document storage | `grantmaestro-documents-uat` | `grantmaestro-documents-prod` |
...
flowchart TB
    ...
    subgraph DNS[GrantMaestro DNS]
        UATAppDNS[app.uat.grantmaestro.com]
        ProdAppDNS[app.grantmaestro.com]
        UATApiDNS[api.uat.grantmaestro.com]
        ProdApiDNS[api.grantmaestro.com]
        MainDomain[grantmaestro.com and www.grantmaestro.com]
    end
...
- `app.uat.grantmaestro.com`, they get the UAT frontend.
- `app.grantmaestro.com`, they get the Production frontend.
...
- UAT frontend calls `api.uat.grantmaestro.com`.
- Production frontend calls `api.grantmaestro.com`.
...
- `https://app.uat.grantmaestro.com`
- `https://app.grantmaestro.com`
...
- Requests for `api.uat.grantmaestro.com` go to the UAT backend service.
- Requests for `api.grantmaestro.com` go to the Production backend service.
...
- UAT database: `grantmaestro_uat`
- Production database: `grantmaestro`
...
- UAT frontend site at `app.uat.grantmaestro.com`
- Production frontend site at `app.grantmaestro.com`
...
- The main domain `grantmaestro.com` and `www.grantmaestro.com` remain on the existing main-domain distribution.
- The app frontend stack manages only `app.uat.grantmaestro.com` and `app.grantmaestro.com`.

### Backend

Built:

- UAT backend API at `api.uat.grantmaestro.com`
- Production backend API at `api.grantmaestro.com`
...
- Use `uat.grantmaestro.com-frontend` for UAT.
- Use `prod.grantmaestro.com-frontend` for Production.
...
- Avoids confusion with the existing main `grantmaestro.com` site.

### Keep `grantmaestro.com` Separate From the App Frontend

Decision:

- Do not move or delete the existing main-domain distribution.
- Serve the app at `app.grantmaestro.com`.
...
- Supports both `api.uat.grantmaestro.com` and `api.grantmaestro.com` cleanly.
...
```bash
curl -I https://app.uat.grantmaestro.com
curl -I https://app.grantmaestro.com
```

Backend:

```bash
curl -i https://api.uat.grantmaestro.com/api/health
curl -i https://api.grantmaestro.com/api/health
```

CORS:

```bash
curl -i -H 'Origin: https://app.uat.grantmaestro.com' https://api.uat.grantmaestro.com/api/health
curl -i -H 'Origin: https://app.grantmaestro.com' https://api.grantmaestro.com/api/health
```

Expected backend health response:

```json
{"database":"ok","domain":"grantmaestro.com","service":"grantmaestro-backend","status":"ok"}
```
